...
//        $order_products = $this->model_checkout_order->getOrderProducts($this->session->data['order_id']);
        $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "order_product WHERE order_id = '" . $this->session->data['order_id']. "'");
        ...
        if ($payment->hasErrors()) {
            ...
            $this->model_checkout_order->addOrderHistory($this->session->data['order_id'], 1); // with 1 always Pending status
    ...
    public function deliveryOrderStatus(&$route, &$data, &$output) {
        ...
        if ( $orderStatus == $this->config->get('payment_zoodpay_complete_status_id'))
        {
            ...
            if (!$query->num_rows) {
                ...
                $post = array(
                    // "delivered_at" => date($this->language->get('date_format_short'), strtotime(time())),
                ...
                if (isset($result['status'])) {
                    // $status_after = $this->db->query("SELECT * FROM " . DB_PREFIX . "order_status WHERE name= '" . $this->config->get('payment_zoodpay_complete_status_id') . "'");
