...
class ModelExtensionPaymentZoodPay extends Model
{
    ...
    public function install()
    {
        ...
        $this->model_extension_event->addEvent('zoodpay_delivery', 'catalog/model/checkout/order/addOrderHistory/after', 'extension/payment/zoodpay/deliveryOrderStatus');
    ...
    public function __construct($registry)
{
    ...
    public function refundOrder($order_id, $comment,  $refund_products, $data, $amount=0)
    {
        ...
        if (isset($result['refund'])) {
            if (isset($result['refund']['declined_reason']) && ($result['refund']['declined_reason'])) {
                ...
                $this->db->query("INSERT INTO " . DB_PREFIX . "order_history SET order_id = '" . (int)$order_id . "', comment = '" . $this->db->escape($result['refund']['declined_reason']) . "', date_added=NOW(), order_status_id= '" . (int)$status_after->row['order_status_id'] . "'");
                ...
                $this->db->query("UPDATE `" . DB_PREFIX . "order` SET order_status_id = '" . $status_after->row['order_status_id'] . "', date_modified = NOW() WHERE order_id = '" . (int)$order_id . "'");
                $this->db->query("INSERT INTO " . DB_PREFIX . "order_history SET order_id = '" . (int)$order_id . "', comment = '" . $this->db->escape($result['refund']['status']) . "', date_added=NOW(), order_status_id= '" . (int)$status_after->row['order_status_id'] . "'");
                $history_id = $this->db->getLastId();
                $this->addRefundHistory($history_id, $order_id, $result['refund_id'],$post['request_id'], $refund_products);
            }
        }
    ...
    public function deliveryOrder($order_id, $comment, $amount, $data)
    {
        ...
        $this->db->query("INSERT INTO " . DB_PREFIX . "order_history SET order_id = '" . (int)$order_id . "', comment = '" . $this->db->escape($comment) . "', date_added=NOW(), order_status_id= '" . $status_before->row['order_status_id'] . "'");
        ...
        if (isset($result['status'])) {
            ...
            $this->db->query("UPDATE `" . DB_PREFIX . "order` SET order_status_id = '" . $status_after->row['order_status_id'] . "', date_modified = NOW() WHERE order_id = '" . (int)$order_id . "'");
            $this->db->query("INSERT INTO " . DB_PREFIX . "order_history SET order_id = '" . (int)$order_id . "', comment = '" . $this->db->escape($result['status']) . "', date_added=NOW(), order_status_id= '" . (int)$status_after->row['order_status_id'] . "'");
        }
    ...
    public function uninstall()
    {
       $this->db->query("DROP TABLE IF EXISTS `" . DB_PREFIX . "zoodpay_configuration`;");
        ...
        $this->load->model('extension/event');
        $this->model_extension_event->deleteEvent('zoodpay_delivery');

        $this->load->model('setting/setting');
        $this->model_setting_setting->deleteSetting('payment_zoodpay');
        $this->model_setting_setting->deleteSetting('zoodpay');
